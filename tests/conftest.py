...
@pytest.fixture
def make_edge() -> Callable[..., AuthEdge]:
    """Factory: create an AuthEdge with sensible defaults, override via kwargs."""
...
def linear_graph() -> nx.DiGraph:
    ...
    for i, (src, dst) in enumerate(edges):
        ...
        g.add_edge(
            ...
            dst,
            edge_id=f"edge-{i}",
            edge_list=[{"edge_id": f"edge-{i}", "edge_type": "ssh"}],
            edge_type="ssh",
        )
    return g
...
def cyclic_graph() -> nx.DiGraph:
    """A→B→C→A: a cycle. DFS via nx.all_simple_paths must not loop."""
    g: nx.DiGraph = nx.DiGraph()
    edges = [("A", "B"), ("B", "C"), ("C", "A")]
    for i, (src, dst) in enumerate(edges):
        g.add_node(src, privilege_tier=0.3, host_id="host:test")
